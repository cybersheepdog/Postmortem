...
# Pool workers live in their own module so `spawn` (Windows, macOS) can
# import them by a real module path. See postmortem/workers.py.
from postmortem.workers import (  # noqa: E402
    _parse_uncached_worker, _deep_analyze_worker, _deep_url_set,
)
...
def parallel_map(worker, items, workers, chunksize):
    ...
    if ProcessPoolExecutor is not None and workers and workers > 1:
        try:
            ...
            try:
                with executor:
                    for result in executor.map(worker, items, chunksize=chunksize):
                        done += 1
                        yield result
                return
            except Exception as exc:  # pragma: no cover - environment dependent
                # A pool that dies mid-run used to abort the whole analysis with
                # a wall of worker tracebacks. Degrading to serial honors this
                # function's contract that parallelism is an optimization.
                # executor.map yields in order, so `done` is a safe resume
                # point: no item is processed twice and none is skipped.
                print(
                    f"\n[!] Process pool failed after {done} of {len(items)} "
                    f"item(s) ({type(exc).__name__}: {exc}).",
                    file=sys.stderr,
                )
                print("[!] Continuing serially. This is slower but complete; "
                      "run with --workers 1 to skip the pool entirely.",
                      file=sys.stderr)
...
def main():
    ...
    print_attachment_diffs(initial_verdict.get("attachment_diffs"))
