...
def print_audit_summary(audit: dict, warnings=None):
    ...
    if _sub_ips:
        print(term.c(f"    from a known attacker subject: {', '.join(_sub_ips)}"
                     ...
                     "red"))
        _sess_ips = d.get("attacker_ips_from_session") or []
        if _sess_ips:
            print(term.c(f"    from a shared session:         {', '.join(_sess_ips)}"
                         "   (same logon as a known attacker address)", "red"))
        _rule_only = [ip for ip in d["attacker_ips"]
                      if ip not in _sub_ips and ip not in _sess_ips]
...
def print_signin_analysis(signin_summary: dict):
    ...
    tr = cov.get("truncation") or {}
    if tr.get("suspected"):
        print(term.c(
            f"  [!] {tr['files_at_cap']} of {tr['files']} file(s) hold exactly "
            f"{tr['page_cap']} records - the export may have stopped at a page "
            f"cap. Everything below is a floor until that is checked.",
            "red", "bold"))
    ...
    if blind:
        ...
        print(term.c("  " + _wrap_indent(
            ...
            "AiTM, legacy authentication and MFA fatigue do not depend on "
            "these fields and are assessed below.",
            2), "red", "bold"))
        return

    _forged = s.get("forged_events") or []
    if _forged:
        print()
        print(term.c(f"  [!] {s.get('forged_count', len(_forged))} record(s) "
                     "carry the markers of a FORGED sign-in event.",
                     "red", "bold"))
        print("  " + _wrap_indent(
            "Events can be injected into this log through the Azure AD "
            "Connect Health pipeline -- from a compromised AD FS server, or a "
            "fake health agent registered by a global administrator, which "
            "works on a cloud-only tenant. The injected timestamp, account "
            "and IP are all attacker-chosen. Compare against AD FS Event ID "
            "1200 on ADDRESS, not on time.", 2))
        for r in _forged[:6]:
            print(f"      {str(r.get('time',''))[:19]:<20} "
                  f"{str(r.get('user',''))[:24]:<24} "
                  f"{str(r.get('ip',''))[:16]:<16} "
                  f"{'; '.join(r.get('markers', []))[:34]}")
        if s.get("forged_count", 0) > 6:
            print(f"      ... {s['forged_count'] - 6} more in the JSON report")
        if s.get("forged_veto_ips"):
            print(term.c("      " + _wrap_indent(
                "%d address(es) were kept out of the victim-address veto as a "
                "result, so a fabricated event cannot exempt the attacker's "
                "own address from attribution."
                % len(s["forged_veto_ips"]), 6), "yellow"))
    ...
    # Counted over every device code sign-in. The line above is downstream of
    # attribution, so it reads 0 whenever attribution failed -- which is not
    # the same claim, and the difference is the whole finding on an export
    # that carries only one leg per group.
    _succ = s.get("device_code_successes", 0)
    if _succ:
        print(f"  Device code sign-ins that succeeded: {_succ}"
              "   (legitimate use included)")
    ...
    lone = s.get("single_leg_findings") or []
    indicated = [r for r in lone if r.get("indicated")]
    review = [r for r in lone if not r.get("indicated")]
    if indicated:
        print()
        print(term.c(f"  {len(indicated)} device code sign-in(s) match the "
                     "single-record profile for device code phishing:",
                     "red", "bold"))
        print("  " + _wrap_indent(
            "Entra records a sign-in where the authentication was initiated, "
            "so a phished flow can be one record carrying the attacker's "
            "address with no second leg to compare. These are assessed on "
            "their own fields -- an indication, not a confirmation.", 2))
        print()
        for r in indicated[:12]:
            print(term.c(f"    {str(r.get('time',''))[:19]:<20} "
                         f"{str(r.get('user',''))[:26]:<26} "
                         f"{str(r.get('ip',''))[:16]:<16} "
                         f"score {r.get('score', 0)}", "red"))
            for why in r.get("reasons", [])[:4]:
                print(f"        - {why}")
        if len(indicated) > 12:
            print(f"    ... {len(indicated) - 12} more in the JSON report")

    if review:
        print()
        print(f"  {len(review)} further device code sign-in(s) worth a look "
              "(below the reporting floor):")
        for r in review[:8]:
            print(f"    {str(r.get('time',''))[:19]:<20} "
                  f"{str(r.get('user',''))[:26]:<26} "
                  f"{str(r.get('ip',''))[:16]:<16} "
                  f"{'; '.join(r.get('reasons', []))[:38]}")
        if len(review) > 8:
            print(f"    ... {len(review) - 8} more in the JSON report")
...
def print_token_replay(replay: dict):
    """Mailbox activity from addresses that never authenticated.

    A token obtained by refreshing an earlier one is invisible to the sign-in
    log -- the access happens and Entra records nothing. The gap is the
    evidence, and it is the only route to an attacker who made no rule and
    never appears in an authentication.
    """
    r = replay or {}
    ...
    if not r.get("replay"):
        return
    print()
    _hdr("TOKEN REPLAY (activity with no matching sign-in)")
    win = r.get("signin_window") or ["", ""]
    print(f"  Sign-in log covers:      {win[0][:19]}  ->  {win[1][:19]}")
    print(f"  Addresses that authenticated: {r.get('authenticated_ips', 0)}")
    print(term.c(f"  Addresses that did NOT:       {r.get('replay_count', 0)}",
                 "red", "bold"))
    if r.get("outside_window"):
        print(f"  Excluded (activity outside that window): {r['outside_window']}")
    print()
    print("  " + _wrap_indent(r.get("note", ""), 2))
    print()
    print(f"    {'address':<18} {'events':>7}  {'first seen':<20} operations")
    print("    " + "-" * 74)
    for row in r["replay"][:12]:
        ops = ", ".join(o for o, _n in (row.get("operations") or [])[:3])
        tag = "  APP" if row.get("app_access") else ""
        print(term.c(f"    {str(row['ip'])[:18]:<18} {row['events']:>7}  "
                     f"{str(row.get('first',''))[:19]:<20} {ops[:24]}{tag}",
                     "yellow" if row.get("app_access") else "red"))
        if row.get("app_access"):
            print(f"      via {row.get('client', '')[:50]}"
                  + (f"   app {', '.join(row['app_ids'][:2])}" if row.get("app_ids") else "")
                  + "   -> check against the OAuth grants")
    if r.get("excused_system"):
        print()
        print(f"  {len(r['excused_system'])} address(es) excused: every operation "
              "from an Exchange background client (RESTSystem/Substrate), which "
              "never signs in.")
    if r["replay_count"] > 12:
        print(f"    ... {r['replay_count'] - 12} more in the JSON report")
...
_PROV_COLOUR = {"observed": "green", "inferred": "yellow",
                "assumed": "cyan", "not assessed": "red"}


def print_case_answers(answers):
    """The nine questions, first. Everything else in the report is workings."""
    if not answers:
        return
    print()
    _hdr("THE CASE IN NINE ANSWERS")
    for a in answers:
        tag = term.c(f"[{a['provenance']}]",
                     _PROV_COLOUR.get(a["provenance"], "white"))
        line = f"  {a['question']:<22} {a['answer']}"
        print((term.c(line, "red", "bold") if a.get("hit") else line) + "  " + tag)
        if a.get("detail"):
            print("      " + _wrap_indent(a["detail"], 6))
        if a.get("gap"):
            print(term.c("      " + _wrap_indent("To answer: " + a["gap"], 6),
                         "yellow"))


def print_sessions(sess: dict, limit: int = 10):
    """One row per attacker logon session -- the unit an intruder works in."""
    s = sess or {}
    if not s.get("available") or not s.get("attacker_sessions"):
        return
    print()
    _hdr("ATTACKER SESSIONS")
    print(f"  Attacker sessions:       {s['attacker_sessions']} of "
          f"{s['sessions_total']} in the log"
          + (f"   ({s['rotated_sessions']} rotated address mid-session)"
             if s.get("rotated_sessions") else ""))
    if s.get("unsessioned_events"):
        print(f"  Events with no SessionId: {s['unsessioned_events']} "
              "(grouped by address only)")
    print()
    print(f"    {'start':<17} {'dur':>5}  {'address(es)':<20} "
          f"{'read':>5} {'del':>4} {'sent':>4} {'rule':>4} {'file':>4}")
    print("    " + "-" * 74)
    for r in s["attacker"][:limit]:
        ips = ", ".join(r["ips"][:2]) + (" +%d" % (len(r["ips"]) - 2)
                                         if len(r["ips"]) > 2 else "")
        reads = str(r["reads"]) + ("*" if r["throttled"] else "")
        line = (f"    {r['first'][:16].replace('T', ' '):<17} {r['minutes']:>4}m  "
                f"{ips[:20]:<20} {reads:>5} {r['deletes']:>4} {r['sends']:>4} "
                f"{r['rules']:>4} {r['files']:>4}")
        print(term.c(line, "red") if r["rotated"] else line)
        if r.get("folders"):
            print("      synced: " + ", ".join(f for f, _n in r["folders"][:3]))
    if s["attacker_sessions"] > limit:
        print(f"    ... {s['attacker_sessions'] - limit} more in the JSON report")
    if any(r["throttled"] for r in s["attacker"]):
        print("    * read count is a floor: the log was throttled in this session")


def print_entry_vectors(s: dict, lures: dict = None, limit: int = 8):
    """AiTM, legacy authentication, MFA fatigue and spray."""
    s = s or {}
    if not s.get("available"):
        return
    aitm = [r for r in (s.get("aitm") or []) if r.get("indicated")]
    lg = s.get("legacy_auth") or {}
    lf = s.get("login_failures") or {}
    if not (aitm or lg.get("indicated_count") or lf.get("fatigue_count")
            or lf.get("spray_count")):
        return
    print()
    _hdr("ENTRY VECTORS (beyond device code)")
    if aitm:
        print(term.c(f"  Adversary-in-the-middle profile: {len(aitm)} sign-in(s)",
                     "red", "bold"))
        for r in aitm[:limit]:
            print(term.c(f"    {str(r.get('time',''))[:19]:<20} "
                         f"{str(r.get('user',''))[:26]:<26} "
                         f"{str(r.get('ip',''))[:16]:<16} score {r.get('score',0)}", "red"))
            for why in r.get("reasons", [])[:3]:
                print(f"        - {why}")
        L = (lures or {}).get("aitm") or {}
        c = L.get("candidates") or []
        if c:
            print(f"    Lure candidates in the corpus (link, arrived within "
                  f"{L.get('window_minutes', 20)} min before):")
            for e in c[:limit]:
                print(f"      {e['arrival'][:19]:<20} -{e['delta_seconds']:>5}s  "
                      f"{(e.get('subject') or '(no subject)')[:40]:<40} "
                      f"{', '.join(e.get('url_domains') or [])[:24]}")
        elif L.get("available"):
            print("    No message with a link arrived in the window before these "
                  "sign-ins; the lure was likely delivered outside email.")
    if lg.get("indicated_count"):
        print()
        print(term.c(f"  Legacy authentication: {lg['indicated_count']} successful "
                     f"sign-in(s) for an affected account or from an attacker "
                     f"address  ({lg['total']} legacy sign-in(s) in the log overall)",
                     "red", "bold"))
        for r in (lg.get("indicated") or [])[:limit]:
            print(f"    {str(r.get('time',''))[:19]:<20} {r.get('user','')[:26]:<26} "
                  f"{r.get('ip','')[:16]:<16} {r.get('protocol','')}"
                  + ("   (attacker address)" if r.get("from_attacker") else ""))
    if lf.get("fatigue_count"):
        print()
        print(term.c(f"  MFA fatigue: {lf['fatigue_count']} pattern(s)", "red", "bold"))
        for r in (lf.get("fatigue") or [])[:limit]:
            print(f"    {r['user'][:26]:<26} from {r['ip']:<16} {r['prompts']} prompt(s) "
                  f"{r['first_prompt'][11:19]}-{r['last_prompt'][11:19]}, approved "
                  f"{r['approved_at'][11:19]} ({r.get('location','')})")
    if lf.get("spray_count"):
        print()
        print(term.c(f"  Password spray: {lf['spray_count']} source address(es)",
                     "yellow", "bold"))
        for r in (lf.get("spray") or [])[:limit]:
            print(f"    {r['ip']:<16} {r['users_hit']} account(s) in one hour, "
                  f"{r['failures']} failure(s) {r['first'][:16]} -> {r['last'][:16]}")


def print_attachment_diffs(diffs: dict, limit: int = 8):
    """The same document, re-sent by name, with different bytes."""
    d = diffs or {}
    if not d.get("count"):
        return
    print()
    _hdr("ATTACHMENTS SUBSTITUTED ON A THREAD")
    print(f"  Re-sent with different contents: {d['count']}   "
          f"(reads as fraud: {d.get('indicated_count', 0)}; "
          f"{d.get('pairs_compared', 0)} thread/name pairs compared)")
    print()
    for e in (d.get("substitutions") or [])[:limit]:
        head = (f"  {e['attachment'][:44]}  score {e['score']}  "
                f"{e['original_arrival'][:16]} -> {e['replacement_arrival'][:16]}")
        print(term.c(head, "red", "bold") if e["indicated"] else head)
        print(f"      original    {e['original_sha256'][:16]}  from {e['original_from']}")
        print(f"      replacement {e['replacement_sha256'][:16]}  from {e['replacement_from']}")
        for why in e.get("reasons", [])[:3]:
            print(f"        - {why}")
    if d["count"] > limit:
        print(f"  ... {d['count'] - limit} more in the JSON report")


def print_searches(sq: dict, limit: int = 12):
    """What the attacker typed into the mailbox search box."""
    s = sq or {}
    if not s.get("available") or not s.get("attacker_searches"):
        return
    print()
    _hdr("WHAT THE ATTACKER SEARCHED FOR")
    print(f"  Searches from attacker addresses: {s['attacker_searches']} "
          f"of {s['total']} in the log   "
          f"({s.get('attacker_intent', 0)} name money, identity or credentials)")
    if s.get("terms"):
        print("  Terms:  " + ", ".join(f"{w} x{n}" for w, n in s["terms"][:8]))
    print()
    print("  " + _wrap_indent(
        "Intent, verbatim. Every other artefact says what they happened to "
        "open; this says what they were looking for.", 2))
    print()
    for r in [x for x in s["searches"] if x["by_attacker"]][:limit]:
        line = (f"    {r['time'][:19]:<20} {r['user'][:22]:<22} "
                f"{r['query'][:44]!r}")
        print(term.c(line, "red", "bold") if r["intent"] else line)
    if s["attacker_searches"] > limit:
        print(f"    ... {s['attacker_searches'] - limit} more in the JSON report")
...
def print_summary(
    ...
    print("-" * 80)
    # Two scores, both shown. PRIORITY is the general suspicion score that
    # orders this list; INITIAL is the initial-email score that decides
    # Tier 1 for inbound mail. A message can sit low here on PRIORITY and
    # still be Tier 1 on INITIAL -- showing one number and tiering on the
    # other read as a contradiction.
    print("    PRIORITY orders this list; INITIAL is the initial-email score "
          "that sets Tier 1. Both are in the CSV as priority_score and "
          "initial_email_score.")
    ...
    for rank, record in enumerate(
        ...
        _head = (f"{rank:2}. PRIORITY {record.score:3} | INITIAL "
                 f"{getattr(record, 'scenario_score', 0):3} | {date} | {sender}")
        ...
        for indicator in (
            ...
            if p:
                ...
                grade = p.get("provenance") or ""
                if grade:
                    tag += " " + term.c(grade, _PROV_COLOUR.get(grade, "white"))
            else:
                tag = ""
    ...
    # Tier 1 is decided on the initial-email score (or on a fact: an
    # attacker-sent subject, an audit hit), so a Tier 1 message can rank
    # below twenty higher-PRIORITY ones and never appear above. Say so,
    # rather than letting the list imply the tier count is what it shows.
    shown = {r.path for r in records_sorted[:20]}
    tier1_below = [r for r in records_sorted[20:]
                   if getattr(r, "tier", 3) == 1 and r.path not in shown]
    if tier1_below:
        print()
        print(term.c(
            f"    {len(tier1_below)} more Tier 1 message(s) rank below the top "
            f"20 on PRIORITY. Sort the HTML candidates table by Tier, or the "
            f"CSV by tier, to see them all.", "yellow"))
...
def _html_record(record):
    ...
    return {
        ...
        "provenance": [
            {k: p.get(k) for k in ("signal", "source", "weight", "severity",
                                   "matched", "provenance")}
...
.table-wrap{overflow:auto;background:#fff;border:1px solid var(--line);border-radius:12px}table{border-collapse:collapse;width:100%;min-width:1250px}th,td{padding:9px;border-bottom:1px solid #eaecf0;text-align:left;vertical-align:top}th{background:#17212b;color:#fff;position:sticky;top:57px}pre{white-space:pre-wrap;max-width:500px}.empty{text-align:center;padding:28px;color:var(--muted)}.remed{background:#fff;border:1px solid var(--line);border-left:4px solid var(--line);border-radius:8px;padding:10px 12px;margin:8px 0;font-size:13px;line-height:1.5}.remed.hit{border-left-color:#d13438}.remed code{display:block;background:#f6f8fa;border-radius:4px;padding:6px 8px;margin-top:6px;font:11px ui-monospace,monospace;white-space:pre-wrap;word-break:break-word}.remed .warn{color:#d13438;display:block;margin-top:6px}.stats{display:flex;flex-wrap:wrap;gap:10px;margin:6px 0 12px}.stat{background:#fff;border:1px solid var(--line);border-radius:8px;padding:8px 12px;min-width:120px}.stat b{display:block;font-size:20px;line-height:1.2}.stat.hit b{color:#d13438}.stat span{font-size:11px;color:var(--muted)}.tbl{width:100%;min-width:0;border-collapse:collapse;font-size:12px;margin-top:6px}.tbl th{text-align:left;border-bottom:1px solid var(--line);padding:5px 8px;font-size:11px;color:var(--muted);font-weight:600;background:transparent;position:static;top:auto}.tbl td{border-bottom:1px solid var(--line);padding:5px 8px;vertical-align:top}.tbl tr.hit td{background:#fff6f6}.tbl code,.remed code.inline{display:inline;background:transparent;padding:0;margin:0;font:11px ui-monospace,monospace;white-space:nowrap}.scroller{max-height:360px;overflow:auto}
.prov{font-size:9px;font-weight:700;letter-spacing:.02em;border-radius:3px;padding:1px 5px;color:#fff;vertical-align:1px}
.prov.observed{background:#1a7f37}.prov.inferred{background:#b26a00}.prov.assumed{background:#0b6e99}.prov.not-assessed{background:#d13438}
.tbl.case{table-layout:fixed;width:100%}
.tbl.case td.q{width:160px;font-weight:600;vertical-align:top;padding-top:9px}
.tbl.case td.ans{vertical-align:top;overflow-wrap:anywhere}.tbl.case td.prov{width:112px;text-align:right;vertical-align:top;padding-top:9px;white-space:nowrap}
.tbl.case tr.hit td.ans b{color:#d13438}
...
<h2>The Case in Nine Answers</h2><div id="execSummary" class="panel"></div>
...
<h2>Attacker Sessions</h2><div id="sessions" class="verdict"></div>
<h2>Exposure Scope (what the attacker read and took)</h2><div id="exposure" class="verdict"></div>
<h2>Attachments Substituted on a Thread</h2><div id="attdiff" class="verdict"></div>
...
function render(){dashboard();execSummary();initialPanel();entryPointPanel();sessionsPanel();exposurePanel();attDiffPanel();remediationPanel();notifyPanel();driftPanel();narrativePanel();verdict();timeline();graph();networkGraph();topDomains();campaigns();candidates();toggleEdges();}
...
function execSummary(){const R=DATA.records||[],total=DATA.total_records||R.length;const t1=R.filter(r=>r.tier===1).length,t2=R.filter(r=>r.tier===2).length;const iv=DATA.initial||{};const A=iv.case_answers||[];const PC={observed:'#1a7f37',inferred:'#b26a00',assumed:'#0b6e99','not assessed':'#d13438'};let body='';if(A.length){body+='<table class="tbl case">'+A.map(a=>`<tr class="${a.hit?'hit':''}"><td class="q">${esc(a.question)}</td><td class="ans"><b>${esc(a.answer)}</b>${a.detail?'<div class="small muted">'+esc(a.detail)+'</div>':''}${a.gap?'<div class="small" style="color:#b26a00">To answer: '+esc(a.gap)+'</div>':''}</td><td class="prov"><span class="badge" style="background:${PC[a.provenance]||'#666'};color:#fff">${esc(a.provenance)}</span></td></tr>`).join('')+'</table>';}body+=`<p class="small muted" style="margin-top:10px">${total} message(s) analysed \u00b7 ${t1} Tier-1 \u00b7 ${t2} Tier-2 \u00b7 scenario ${esc(iv.scenario||'n/a')}${iv.initial_email?' \u00b7 <button class="small" onclick="focusPath('+js(iv.initial_email.path)+')">initial email</button>':''}</p>`;document.getElementById('execSummary').innerHTML=body;}
...
function entryPointPanel(){const el=document.getElementById('entrypoint');if(!el)return;const v=DATA.initial||{};const s=v.signin_log||{};const L=v.signin_lures||{};if(!s.available){el.innerHTML='<div class="empty">No Entra sign-in log was supplied. Device code flow phishing leaves no trace in the mail itself, so this cannot be assessed from the corpus.</div>';return;}const cov=s.coverage||{};let b='';const tr=cov.truncation||{};if(tr.suspected){b+='<div class="remed hit"><b>The sign-in export may be truncated.</b> '+tr.files_at_cap+' of '+tr.files+' file(s) hold exactly '+tr.page_cap+' records \u2014 the shape of an exporter stopping at a per-request page cap rather than at the end of the data. Every baseline, AiTM profile and token-replay window on this page is a floor until the exporter is confirmed to have followed @odata.nextLink.</div>';}if(cov.blind){b+='<div class="remed"><b>Device code flow cannot be assessed from this export.</b> It carries neither originalTransferMethod nor authenticationProtocol. A clean device code result here is not a negative. The other entry vectors below do not depend on those fields.</div>';}const fg=s.forged_events||[];if(fg.length){b+='<div class="remed hit"><b>'+(s.forged_count||fg.length)+' sign-in record(s) carry the markers of a FORGED event.</b> Events can be injected into this log through the Azure AD Connect Health pipeline \u2014 from a compromised AD FS server, or a fake health agent registered by a global administrator, which works on a cloud-only tenant. The injected timestamp, account and IP are all attacker-chosen, so every conclusion below is provisional until AD FS Event ID 1200 is compared against it \u2014 on address, not on time.'+((s.forged_veto_ips||[]).length?'<br><span class="small">'+s.forged_veto_ips.length+' address(es) were kept out of the victim-address veto as a result, so a fabricated event cannot exempt the attacker\u2019s own address from attribution.</span>':'')+'</div>';}b+='<div class="stats">'+stat(s.device_code_records||0,'device code sign-ins')+stat(s.attack_groups||0,'location splits',(s.attack_groups||0)>0)+stat(s.device_code_successes||0,'succeeded')+stat(s.indicated_count||0,'match phishing profile',(s.indicated_count||0)>0)+stat(s.tokens_issued||0,'attributed to attacker',(s.tokens_issued||0)>0)+stat((s.attacker_ips||[]).length,'attacker addresses',(s.attacker_ips||[]).length>0)+'</div>';if(s.earliest_token)b+=`<p class="small"><b>First token issued:</b> <code class="inline">${esc(s.earliest_token)}</code> \u2014 the true compromise T0, earlier than the first recorded action.</p>`;(L.confirmed||[]).forEach(e=>{b+=`<div class="remed hit">`+`<b>Lure:</b> ${esc(e.subject||'(no subject)')}<br>`+`<span class="small muted">from ${esc(e.sender||'')} \u00b7 arrived `+`${esc(e.arrival||'')}</span><br>`+`<span class="small">Token issued ${esc(e.token_time||'')} `+`(${Number(e.delta_seconds||0)}s later) from ${esc(e.attacker_ip||'')}`+(e.code?` \u00b7 code <code class="inline">${esc(e.code)}</code>`:'')+`</span>`+`</div>`;});if(!(L.confirmed||[]).length&&(L.tokens_without_lure||[]).length)b+='<p class="small" style="color:#d13438">A token was issued but no message '+'in the corpus carries a device login URL in the window before it. The lure '+'was most likely delivered outside email \u2014 Teams chat is the common '+'channel \u2014 and that data should be requested.</p>';(s.groups||[]).filter(g=>g.verdict==='attack').forEach(g=>{b+=`<div class="remed hit"><b>${esc(g.user||'')}</b> `+`<span class="small muted">differs by ${esc((g.differs||[]).join(', '))}</span>`+'<table class="tbl"><tr><th>time</th><th>address</th><th>location</th><th>leg</th></tr>'+(g.legs||[]).map(l=>`<tr class="${l.interactive?'':'hit'}"><td>${esc(l.time||'')}</td><td><code>${esc(l.ip||'')}</code></td><td>${esc(l.location||'')}</td><td>${l.interactive?'victim (interactive)':'polling client'}</td></tr>`).join('')+'</table></div>';});const ai=(s.aitm||[]).filter(r=>r.indicated);const lg=s.legacy_auth||{};const lf=s.login_failures||{};const AL=(L.aitm||{});if(ai.length){b+='<p class="small"><b>'+ai.length+' sign-in(s) fit the adversary-in-the-middle profile</b>: no fresh authentication performed, from an ASN or country this account had never used. The victim authenticated for real through a proxy; this is the attacker replaying the result.</p>'+'<div class="scroller"><table class="tbl"><tr><th>time</th><th>account</th><th>address</th><th>location</th><th>score</th><th>why</th></tr>'+ai.map(r=>`<tr class="hit"><td>${esc(r.time||'')}</td><td>${esc(r.user||'')}</td><td><code>${esc(r.ip||'')}</code></td><td>${esc(r.location||'')}</td><td>${Number(r.score||0)}</td><td>${esc((r.reasons||[]).join('; '))}</td></tr>`).join('')+'</table></div>';const lc=AL.candidates||[];if(lc.length){b+='<p class="small"><b>Lure candidates</b> (carry a link, arrived within '+(AL.window_minutes||20)+' min before):</p><table class="tbl"><tr><th>arrived</th><th>before</th><th>subject</th><th>from</th><th>links to</th></tr>'+lc.slice(0,12).map(e=>`<tr><td>${esc((e.arrival||'').slice(0,19))}</td><td>${Number(e.delta_seconds||0)}s</td><td><button class="small" onclick="focusPath(${js(e.path)})">${esc(e.subject||'(no subject)')}</button></td><td>${esc(e.sender||'')}</td><td>${esc((e.url_domains||[]).join(', '))}</td></tr>`).join('')+'</table>';}}if(lg.indicated_count){b+='<p class="small"><b>'+lg.indicated_count+' successful legacy-protocol sign-in(s)</b> for an affected account or from an attacker address ('+esc((lg.protocols||[]).slice(0,3).map(p=>p[0]).join(', '))+'). Legacy authentication never asks for MFA.</p>'+'<table class="tbl"><tr><th>time</th><th>account</th><th>address</th><th>protocol</th></tr>'+(lg.indicated||[]).slice(0,12).map(r=>`<tr class="hit"><td>${esc(r.time||'')}</td><td>${esc(r.user||'')}</td><td><code>${esc(r.ip||'')}</code></td><td>${esc(r.protocol||'')}</td></tr>`).join('')+'</table>';}if(lf.fatigue_count){b+='<p class="small"><b>'+lf.fatigue_count+' MFA-fatigue pattern(s)</b>: a run of prompts for one account from one address, then an approval.</p>'+'<table class="tbl"><tr><th>account</th><th>address</th><th>prompts</th><th>approved</th><th>where</th></tr>'+(lf.fatigue||[]).slice(0,8).map(r=>`<tr class="hit"><td>${esc(r.user)}</td><td><code>${esc(r.ip)}</code></td><td>${Number(r.prompts)}</td><td>${esc((r.approved_at||'').slice(0,19))}</td><td>${esc(r.location||'')}</td></tr>`).join('')+'</table>';}if(lf.spray_count){b+='<p class="small"><b>'+lf.spray_count+' password-spray source(s)</b>: wrong-password failures across many accounts from one address inside an hour.</p>';}const all=s.single_leg_findings||[];const lone=all.filter(r=>r.indicated);const rep=s.token_replay||{};if(rep.assessed&&(rep.replay||[]).length){b+='<p class="small"><b>'+rep.replay.length+' address(es) acted on the mailbox without ever authenticating.</b> A token obtained by refreshing an earlier one is invisible to the sign-in log: the access happens and Entra records nothing. The gap is the evidence.</p>'+'<div class="scroller"><table class="tbl"><tr><th>address</th><th>events</th><th>first seen</th><th>operations</th><th>client</th></tr>'+rep.replay.map(r=>`<tr class="${r.app_access?'':'hit'}"><td><code>${esc(r.ip||'')}</code></td><td>${Number(r.events||0)}</td><td>${esc((r.first||'').slice(0,19))}</td><td>${esc((r.operations||[]).slice(0,3).map(o=>o[0]).join(', '))}</td><td class="small">${r.app_access?'<b>APP</b> ':''}${esc(r.client||'')}${(r.app_ids||[]).length?'<br>app '+esc(r.app_ids.slice(0,2).join(', '))+' \u2014 check OAuth grants':''}</td></tr>`).join('')+'</table></div>';if((rep.excused_system||[]).length)b+='<p class="small muted">'+rep.excused_system.length+' address(es) excused: every operation from an Exchange background client, which never signs in.</p>';}if(lone.length){b+='<p class="small"><b>'+lone.length+' device code sign-in(s) match the single-record profile for device code phishing.</b> Entra records a sign-in where the authentication was initiated, so a phished flow can be one record carrying the attacker\u2019s address with no second leg to compare. These are assessed on their own fields \u2014 an indication, not a confirmation.</p>'+'<div class="scroller"><table class="tbl"><tr><th>time</th><th>account</th><th>address</th><th>location</th><th>score</th><th>why</th></tr>'+lone.map(r=>`<tr class="hit"><td>${esc(r.time||'')}</td><td>${esc(r.user||'')}</td><td><code>${esc(r.ip||'')}</code></td><td>${esc(r.location||'')}</td><td>${Number(r.score||0)}</td><td>${esc((r.reasons||[]).join('; '))}</td></tr>`).join('')+'</table></div>';}(s.warnings||[]).forEach(w=>{b+=`<p class="small" style="color:#b26a00">${esc(w)}</p>`;});el.innerHTML=b;}
function stat(n,label,hit){return `<div class="stat ${hit?'hit':''}"><b>${esc(String(n))}</b><span>${esc(label)}</span></div>`;}
function sessionsPanel(){const el=document.getElementById('sessions');if(!el)return;const s=((DATA.initial||{}).audit_log||{}).sessions||{};if(!s.available){el.innerHTML='<div class="empty">No audit log, or no SessionId on its events. Sessions need Exchange audit records from 2020 onward.</div>';return;}if(!s.attacker_sessions){el.innerHTML='<div class="empty">'+s.sessions_total+' session(s) in the log; none contains an attacker address.</div>';return;}let b='<div class="stats">'+stat(s.attacker_sessions,'attacker sessions',true)+stat(s.sessions_total,'sessions in log')+stat(s.rotated_sessions||0,'rotated address mid-session',(s.rotated_sessions||0)>0)+stat(s.unsessioned_events||0,'events without SessionId')+'</div>';b+='<p class="small muted">The unit an intruder works in. An address that shares a session with a known attacker address is the same logon, which is how rotation through a residential proxy is joined back together.</p>';b+='<div class="scroller"><table class="tbl"><tr><th>start</th><th>dur</th><th>address(es)</th><th>read</th><th>deleted</th><th>sent</th><th>rules</th><th>files</th><th>client</th></tr>'+(s.attacker||[]).map(r=>`<tr class="hit"><td>${esc((r.first||'').slice(0,16).replace('T',' '))}</td><td>${Number(r.minutes||0)}m</td><td><code>${esc((r.ips||[]).join(', '))}</code>${r.rotated?' <b>rotated</b>':''}</td><td>${Number(r.reads||0)}${r.throttled?' <b>*</b>':''}${r.sync_reads?' <span class="small muted">('+r.sync_reads+' sync)</span>':''}</td><td>${Number(r.deletes||0)}</td><td>${Number(r.sends||0)}</td><td>${Number(r.rules||0)}</td><td>${Number(r.files||0)}</td><td class="small">${esc(r.client||'')}</td></tr>`).join('')+'</table></div>';if((s.attacker||[]).some(r=>r.throttled))b+='<p class="small">* read count is a floor: the log was throttled in this session.</p>';el.innerHTML=b;}
function attDiffPanel(){const el=document.getElementById('attdiff');if(!el)return;const d=(DATA.initial||{}).attachment_diffs||{};if(!d.available){el.innerHTML='<div class="empty">No thread carried the same attachment more than once.</div>';return;}if(!d.count){el.innerHTML='<div class="empty">'+d.pairs_compared+' thread/name pair(s) compared; every re-sent attachment was byte-identical.</div>';return;}let b='<div class="stats">'+stat(d.count,'re-sent with different contents',d.count>0)+stat(d.indicated_count||0,'read as fraud',(d.indicated_count||0)>0)+stat(d.pairs_compared||0,'pairs compared')+'</div>';b+='<p class="small muted">The same document, on the same thread, sent twice with different bytes. No single message looks wrong; the change is the finding. Open both copies and compare the bank details.</p>';b+='<div class="scroller"><table class="tbl"><tr><th>attachment</th><th>original</th><th>replacement</th><th>score</th><th>why</th></tr>'+(d.substitutions||[]).map(e=>`<tr class="${e.indicated?'hit':''}"><td>${esc(e.attachment)}</td><td><button class="small" onclick="focusPath(${js(e.original_path)})">${esc((e.original_arrival||'').slice(0,16))}</button><div class="small muted">${esc(e.original_from)}</div></td><td><button class="small" onclick="focusPath(${js(e.replacement_path)})">${esc((e.replacement_arrival||'').slice(0,16))}</button><div class="small muted">${esc(e.replacement_from)}</div></td><td>${Number(e.score||0)}</td><td class="small">${esc((e.reasons||[]).join('; '))}</td></tr>`).join('')+'</table></div>';el.innerHTML=b;}
function exposurePanel(){const el=document.getElementById('exposure');if(!el)return;const v=DATA.initial||{};const x=v.exposure_scope||{};const a=v.audit_log||{};const fa=a.file_activity||{};const da=a.delegate_access||{};if(!x.available&&!fa.available){el.innerHTML='<div class="empty">'+esc(x.reason||'No audit log was supplied, so what the attacker read cannot be established.')+'</div>';return;}let b='';if(x.scope_is_lower_bound){b+='<div class="remed hit"><b>The read count below is a LOWER BOUND.</b> '+esc(x.scope_note||'')+'</div>';}b+='<div class="stats">'+stat(x.messages_read||0,'messages read'+(x.scope_is_lower_bound?' (floor)':''),(x.messages_read||0)>0)+stat(x.read_by_sync||0,'via folder Sync',(x.read_by_sync||0)>0)+stat(x.read_by_bind||0,'opened individually')+stat(x.read_with_attachments||0,'with attachment')+stat(x.throttled_events||0,'throttled events',(x.throttled_events||0)>0)+stat(fa.exfil_count||0,'files downloaded',(fa.exfil_count||0)>0)+stat(fa.shared_count||0,'shared outward',(fa.shared_count||0)>0)+'</div>';if((x.synced_folders||[]).length){b+='<p class="small"><b>Folders pulled down whole:</b> '+esc(x.synced_folders.map(f=>f[0]+' (x'+f[1]+')').join(', '))+' &mdash; every item in each is exposed, not only the ones enumerated.</p>';}else if(x.scope_note&&!x.scope_is_lower_bound){b+='<p class="small">'+esc(x.scope_note)+'</p>';}if((x.read||[]).length){b+='<div class="scroller"><table class="tbl"><tr><th>first access</th><th>how</th><th>subject</th><th>from</th></tr>'+x.read.slice(0,200).map(r=>`<tr class="hit"><td>${esc((r.first_access||'').slice(0,19))}</td><td>${esc(r.access_type||'')}${r.throttled?' <b>throttled</b>':''}</td><td>${esc(r.subject||'(subject not recorded)')}${r.has_attachment?' \ud83d\udcce':''}</td><td>${esc(r.sender||'')}</td></tr>`).join('')+'</table></div>';if(x.read.length>200)b+='<p class="small muted">'+(x.read.length-200)+' more in the JSON report.</p>';}const sq=a.searches||{};const asq=(sq.searches||[]).filter(r=>r.by_attacker);if(asq.length){b+='<p class="small" style="margin-top:14px"><b>What the attacker searched for:</b> '+asq.length+' search(es) from attacker addresses'+(sq.attacker_intent?', <b>'+sq.attacker_intent+'</b> naming money, identity or credentials':'')+'. Intent, verbatim.</p>'+'<div class="scroller"><table class="tbl"><tr><th>time</th><th>account</th><th>query</th><th>terms</th></tr>'+asq.slice(0,40).map(r=>`<tr class="${(r.intent||[]).length?'hit':''}"><td>${esc((r.time||'').slice(0,19))}</td><td>${esc(r.user||'')}</td><td><code>${esc(r.query||'')}</code></td><td class="small">${esc((r.intent||[]).join(', '))}</td></tr>`).join('')+'</table></div>';}if(fa.available&&(fa.exfil_count||fa.shared_count)){b+='<p class="small" style="margin-top:14px"><b>SharePoint / OneDrive from attacker addresses:</b> '+Number(fa.attacker_file_events||0)+' event(s)'+(fa.full_sync?', <b>'+fa.full_sync+' full-drive sync(s)</b>':'')+'.</p>'+'<div class="scroller"><table class="tbl"><tr><th>time</th><th>operation</th><th>file</th></tr>'+(fa.exfil||[]).concat(fa.shared||[]).map(r=>`<tr class="hit"><td>${esc((r.time||'').slice(0,19))}</td><td>${esc(r.operation||'')}</td><td>${esc(r.file||r.path||'')}</td></tr>`).join('')+'</table></div>';}const unk=(da.principals||[]).filter(p=>!p.known_delegate||p.from_attacker_ip);if(unk.length){b+='<p class="small" style="margin-top:14px"><b>Delegate access by principals holding no known delegation:</b>'+(da.delegates_known?'':' <span class="muted">(no permissions export supplied \u2014 unverified, not unauthorised)</span>')+'</p>'+'<table class="tbl"><tr><th>principal</th><th>on</th><th>events</th><th>from attacker IP</th></tr>'+unk.map(p=>`<tr class="hit"><td>${esc(p.principal)}</td><td>${esc((p.mailboxes||[]).join(', '))}</td><td>${Number(p.events||0)}</td><td>${p.from_attacker_ip?'yes':''}</td></tr>`).join('')+'</table>';}el.innerHTML=b;}
...
function remediationPanel(){const el=document.getElementById('remediation');if(!el)return;const v=DATA.initial||{};const acts=v.remediation||[];const p=v.persistence||{};if(!acts.length){const w=(p.warnings||[]);el.innerHTML=w.length?`<div class="verdict-title">NOT ASSESSED</div>`+w.map(x=>`<p class="small">${esc(x)}</p>`).join(''):'<div class="empty">No persistence mechanism was identified. '+'Supply --mes-dir to assess whether the attacker retains access: an '+'OAuth consent grant leaves no trace in mail or sign-in data.</div>';return;}const both=acts.filter(a=>a.survives_password_reset==='survives'&&a.survives_token_revocation==='survives').length;const tw=(p.tenant_wide||[]);let body=tw.length?('<div class="remed hit"><b>'+tw.length+' TENANT-WIDE mechanism(s): '+esc(tw.map(x=>String(x.kind).replace(/_/g," ")).join(', '))+'.</b> These are not scoped to an account. A password reset, a token revocation, MFA re-registration and deleting the compromised user all leave them working.</div>'):'';body+=`<div class="verdict-title">${acts.length} ACTION(S) REQUIRED`+(both?` \u00b7 ${both} NOT fixed by a password reset or token revocation`:'')+`</div>`;body+='<p class="small muted">Ordered by what survives the response most '+'clients have already made.</p>';body+=acts.map(a=>`<div class="remed ${a.by_attacker?'hit':''}">`+`<b>${a.priority}. ${esc(String(a.kind).replace(/_/g,' '))}:</b> ${esc(a.target||'')}${a.provenance?` <span class="prov ${esc(a.provenance)}">${esc(a.provenance)}</span>`:''}${a.by_attacker&&a.attribution_grade?` <span class="prov ${esc(a.attribution_grade)}" title="attribution">attribution ${esc(a.attribution_grade)}</span>`:''}`+(a.when?`<br><span class="small muted">${esc(a.when)}`+(a.attribution?` \u2014 ${esc(a.attribution)}`:'')+`</span>`:'')+(a.detail?`<br><span class="small">${esc(a.detail)}</span>`:'')+(a.grants?`<br><span class="small">${esc(a.grants)}</span>`:'')+`<br><code class="small">${esc(a.action||'')}</code>`+(a.not_fixed_by?`<span class="warn small">${esc(a.not_fixed_by)}</span>`:'')+`</div>`).join('');el.innerHTML=body;}
function initialPanel(){const v=DATA.initial||{};const e=v.initial_email;const el=document.getElementById('initial');if(!el)return;const LI=v.lookalike_infrastructure||{};let pre='';if(LI.indicated_count){pre+='<div class="remed hit"><b>'+LI.indicated_count+' purpose-built lookalike domain(s).</b> Each resembles a known domain AND was registered recently \u2014 two of the strongest signals in the corpus, joined. Infrastructure made for this case.'+'<table class="tbl"><tr><th>domain</th><th>resembles</th><th>age</th><th>messages</th><th>why</th></tr>'+(LI.domains||[]).filter(g=>g.indicated).map(g=>`<tr class="hit"><td><code>${esc(g.domain)}</code></td><td><code>${esc(g.resembles)}</code>${g.resembles_victim?' <b>(victim)</b>':''}</td><td>${Number(g.age_days)}d</td><td>${Number(g.messages)}</td><td class="small">${esc((g.reasons||[]).join('; '))}</td></tr>`).join('')+'</table></div>';}let body=`<div class="verdict-title">${esc(v.verdict||'NO INITIAL EMAIL IDENTIFIED')} · ${esc(v.scenario||'')} · confidence ${esc(v.confidence||'low')}</div><p class="small muted">${esc(v.scenario_reason||'')}</p>`;if(e){body+=`<p><b>Initial email:</b> <button class="small" onclick="focusPath(${js(e.path)})">${esc(e.subject||'(no subject)')}</button><br><b>Timestamp:</b> ${esc(e.timestamp||'')}<br><b>Sender:</b> ${esc(e.sender||'')}<br><b>Stage:</b> ${esc(e.stage||'')}<br><b>Initial-email score:</b> ${Number(e.initial_score||0)} (priority ${Number(e.priority_score||0)})</p>`;if((e.anchor_matches||[]).length)body+=`<p class="risk">Anchor matches: ${(e.anchor_matches||[]).map(esc).join(', ')}</p>`;const FV={};(e.findings||[]).forEach(f=>{FV[f.signal]=f;});body+=`<b>Why</b><ul class="indicator">${(e.reasons||[]).map(x=>{const f=FV[x]||{};const ev=f.matched?`<div class="small muted">evidence: <code>${esc(String(f.matched))}</code>${f.source?` &middot; ${esc(f.source)}`:''}${f.weight?` &middot; ${f.weight>0?'+':''}${f.weight}`:''}</div>`:'';return `<li>${esc(x)}${ev}</li>`;}).join('')||'<li>&mdash;</li>'}</ul>`;}body+=`<b>Ranked candidates</b>${(v.shortlist||[]).map(x=>`<div class="small"><button class="small" onclick="focusPath(${js(x.path)})">[${Number(x.initial_score||0)}] ${esc(x.timestamp||'')} · ${esc(x.sender||'')} · ${esc(x.subject||'(no subject)')}</button></div>`).join('')||'<span class="muted">None</span>'}`;el.innerHTML=pre+body;}
