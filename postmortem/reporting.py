...
def print_signin_analysis(signin_summary: dict):
    ...
    if lone:
        print()
        print(term.c(f"  {len(lone)} lone device code sign-in(s) with two or more "
                     "reasons to look wrong:", "yellow", "bold"))
        print("  " + _wrap_indent(
            "Not attributed. Without both legs of the flow there is no way to "
            "tell the victim's authentication from the attacker's polling "
            "client, so this is a review list, not a finding.", 2))
        print()
        print(f"    {'time':<20} {'account':<28} {'address':<16} why")
        print("    " + "-" * 74)
        for r in lone[:12]:
            print(f"    {str(r.get('time',''))[:19]:<20} "
                  f"{str(r.get('user',''))[:28]:<28} "
                  f"{str(r.get('ip',''))[:16]:<16} "
                  f"{'; '.join(r.get('reasons', []))[:40]}")
        if len(lone) > 12:
            print(f"    ... {len(lone) - 12} more in the JSON report")
