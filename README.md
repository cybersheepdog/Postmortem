...
python bec_hunt.py ./mailbox_export

# A Gmail/Unix MBOX or an Outlook PST, with an HTML + JSON report
python bec_hunt.py ./case.mbox -o report.html --json report.json

# The high-value run: mailbox + M365 audit log together
python bec_hunt.py ./mailbox.pst --audit-log ./UnifiedAuditLog.csv -o report.html
```
...
Containers are exploded into `.eml` under `<scan root>/.bec_hunt_extracted/…`, **preserving the source folder hierarchy in the path** so folder-based concealment detection works exactly as it does on a raw `.eml` tree. Extraction is idempotent (reuses prior output unless `--reingest`).
...
From it, `bec_hunt` derives the compromise time, attacker IP(s), forwarding address/domain, and rule keywords, and turns the malicious mailbox rule and attacker sign-ins into **confirmed** evidence in the attack narrative. (Attacker IPs come from the `ClientIP` that created the malicious rule; any `UserLoggedIn` from that IP is then treated as an attacker session.)
...
Rather than trusting any single header, `bec_hunt` combines config-independent, attacker-controlled signals:
...
`--config file.json` overrides scoring weights, thresholds, clustering caps, and baseline minimums (see `CONFIG` in [`bechunt/config.py`](bechunt/config.py) for the keys). The effective values are recorded in the run manifest.
...
python -m pyflakes bec_hunt.py bechunt/*.py   # lint
```

### Project layout

```
bec_hunt.py            CLI orchestrator (argument parsing, run wiring)
bechunt/
